...
group_two.add_argument(
    '--deploy-vm', help='Build a virtual machine, from a filesystem tarball available locally', nargs='?')
group_two.add_argument(
    '--deploy-vm-remote', help='Build a virtual machine, from a tarball hosted on Artifactory', nargs='?')
group_two.add_argument('--delete-vm', help='Delete a deployed VM', nargs='?')
group_two.add_argument(
    '--delete-vm-host', help='Delete a deployed VM', nargs='?')
group_two.add_argument(
    '--upload', help='Upload the archive file to Artifactory', action='store_true')
group_two.add_argument('--generate-test-rpm',
                       help='Build and upload the test rpm', action='store_true')

# Optional arguments
arg_parser.add_argument(
    '--verbose', help='Add verbosity to the ansible playbook execution', action='store_true')
arg_parser.add_argument(
...
arg_parser.add_argument(
    '--build-id', help='Add a build ID to the archive when triggered by a build system', required=True, nargs='?')
